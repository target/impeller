...
func TestReadConfigWithShellAndSecrets(t *testing.T) {
	...
	require.Len(t, release.Shell, 1)
	assert.Equal(t, "echo ready", release.Shell[0])
	...
	assert.Equal(t, "plain-password", release.Secrets[0].Data["password"])
}
